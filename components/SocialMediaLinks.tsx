...
export const socialMediaItems = [
  {
    href: "https://www.youtube.com/@ItsLifey-co",
    label: "It's Lifey on YouTube",
    Icon: IconYouTube,
  },
  {
    href: "https://www.instagram.com/itslifeyco/",
    label: "It's Lifey on Instagram",
    Icon: IconInstagram,
  },
  {
    href: "https://www.facebook.com/profile.php?id=61573198074373",
    label: "It's Lifey on Facebook",
    Icon: IconFacebook,
  },
] as const;
...
type SocialMediaLinksProps = {
  ...
  className?: string;
  /** Applied to each social anchor */
  linkClassName: string;
  /** Pixel size for the icon glyph (width/height) */
...
export function SocialMediaLinks({
  className,
  linkClassName,
  iconSizeClassName = "h-[1.35rem] w-[1.35rem]",
}: SocialMediaLinksProps) {
  return (
    <nav aria-label="Social media" className={className}>
      {socialMediaItems.map(({ href, label, Icon }) => (
        <a
          key={href}
          href={href}
          target="_blank"
        ...
        >
          <Icon className={iconSizeClassName} aria-hidden />
        </a>
      ))}
    </nav>
  );
